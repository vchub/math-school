...
### Практика: Пути с ограничениями

**Пример (сетка с препятствием):**
Представьте сетку $3 \times 3$. Но в самой центральной точке (координаты (1, 1), если считать от 0) стоит забор, через который нельзя пройти. Сколько существует путей из $S(0,0)$ в $E(2,2)$?

**Решение:**
Если мы попробуем применить общую формулу $\frac{(2+2)!}{2!2!} = 6$, мы получим ответ для пустой сетки. Но наш путь не может проходить через центр.
Используем **Метод сложения в узлах**:

- В узлы $(0,1)$ и $(1,0)$ ведет по **1** пути.
- В узел $(1,1)$ ведет $1+1=2$ пути, но он **заблокирован**, поэтому ставим там **0**.
- В узел $(2,1)$ теперь можно попасть только сверху (из $(2,0)$), так как слева забор. $1 + 0 = 1$ путь.
- Аналогично для $(1,2)$ — только $1 + 0 = 1$ путь.
- В финальную точку $E(2,2)$ ведут пути из $(2,1)$ и $(1,2)$. Итого: $1 + 1 = 2$ пути.

**Упражнение 35:** Найдите количество различных путей от $S$ до $E$ в сетке размером $6 \times 3$ (6 шагов вправо, 3 шага вниз).

**Упражнение 36:** Робот-доставщик на складе должен проехать из угла $A$ в противоположный угол $B$. Склад разбит на кварталы сеткой $4 \times 5$. Робот может двигаться только на север или на восток. Сколько у него вариантов маршрута?

**Упражнение 37 (Нетипичная задача):**
В саду проложены дорожки в виде сетки $5 \times 5$ (5 шагов вправо, 5 шагов вниз). Однако центральный перекресток (3-й шаг вправо, 3-й шаг вниз) перекопан, и проходить через него нельзя. Сколько путей существует от старта до финиша?

_Подсказка: Попробуйте заполнить сетку числами, как в треугольнике Паскаля, ставя 0 там, где проход закрыт._
...
<!-- **Ответы:** -->
<!-- 32. $\frac{(a+b)!}{a!b!}$ для всей сетки. -->
<!-- 35. 84 пути. -->
<!-- 36. 126 путей. -->
<!-- 37. 132 пути. (В пустой сетке было бы 252, но через центр (3,3) проходит 120 путей. $252 - 120 = 132$). -->
